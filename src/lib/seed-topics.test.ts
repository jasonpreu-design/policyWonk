...
describe("Seed Topics", () => {
  ...
  it("creates parent-child relationships", () => {
    seedTopics(db);
    const healthcare = db.prepare("SELECT id FROM topics WHERE domain = 'Healthcare' AND parent_id IS NULL").get() as any;
    const children = db.prepare("SELECT COUNT(*) as count FROM topics WHERE parent_id = ?").get(healthcare.id) as any;
    expect(children.count).toBe(7);
  });

  it("sets sort_order on domains and subtopics", () => {
    seedTopics(db);
    const domains = db.prepare("SELECT name, sort_order FROM topics WHERE parent_id IS NULL ORDER BY sort_order").all() as any[];
    ...
    const healthcare = db.prepare("SELECT id FROM topics WHERE domain = 'Healthcare' AND parent_id IS NULL").get() as any;
    const subs = db.prepare("SELECT name, sort_order FROM topics WHERE parent_id = ? ORDER BY sort_order").all(healthcare.id) as any[];
  ...
  it("stores domain field on subtopics matching parent", () => {
    seedTopics(db);
    const subtopics = db.prepare("SELECT domain FROM topics WHERE parent_id IS NOT NULL AND domain = 'Immigration'").all() as any[];
    expect(subtopics.length).toBe(7);
  });

  it("stores descriptions on all topics", () => {
    seedTopics(db);
    const missing = db.prepare("SELECT COUNT(*) as count FROM topics WHERE description IS NULL OR description = ''").get() as any;
